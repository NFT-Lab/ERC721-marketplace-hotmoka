...
public abstract class TakamakaTest {
    ...
    protected final Path erc721CodePath =
            Paths.get(
                    System.getProperty("user.home")
                            + "/.m2/repository/io/nfteam/nftlab/hotmoka-erc721/0.1/hotmoka-erc721-0.1.jar");

    protected final Path smartContractCodePath = Paths.get("./target/erc721-marketplace-0.1.jar");

    /**
     * The node that gets created before starting running the tests. This node will hence be created
     * only once and each test will decorate it into {@linkplain #nodeWithAccountsView}, with the
     * addition of the jar and accounts that the test needs.
     */
    ...
    private static final PrivateKey privateKeyOfLocalGamete;

    /** The account that can be used as gamete for each run of the tests. */
    protected static final StorageReference localGamete;

    /** The signature algorithm used for signing the requests. */
    private static final SignatureAlgorithm<SignedTransactionRequest> signature;

    /** The jar under test. */
    private static TransactionReference jar;

    /**
     * The node under test. This is a view of {@linkplain #node}, with the addition of some initial
     * accounts, recreated before each test.
     */
    ...
    protected static final String chainId;
    ...
    /**
     * Non-null if the node is based on AILIA's Takamaka blockchain, so that a specific
     * initialization can be run.
     */
    protected static TakamakaBlockchain takamakaBlockchain;

    /** The version of the Takamaka project, as stated in the pom file. */
    private static final String takamakaVersion;

    /** The version of the Hotmoka project, as stated in the pom file. */
    private static final String hotmokaVersion;

    private static final Logger logger = LoggerFactory.getLogger(TakamakaTest.class);
    ...
    static {
        try {
            ...
            StorageReference gamete =
                    (StorageReference)
                            node.runInstanceMethodCallTransaction(
            ...
            node.addInstanceMethodCallTransaction(
                    new InstanceMethodCallTransactionRequest(
                            Signer.with(signature, privateKeyOfGamete),
                            gamete,
                            nonce,
                            chainId,
                            _100_000,
                            ...
                            new VoidMethodSignature(
                                    ...
                                    ClassType.BIG_INTEGER),
                            gamete,
                            new BigIntegerValue(aLot),
                            new BigIntegerValue(aLot)));

            NodeWithAccounts local =
                    NodeWithAccounts.ofGreenRed(node, gamete, privateKeyOfGamete, aLot, aLot);
    ...
    private static void dumpKeys(KeyPair keys) throws IOException {
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("gamete.keys"))) {
            ...
            System.out.println(
                    "The keys of the gamete have been succesfully written into the file gamete.keys");
        }
    }

    private static KeyPair loadKeysOfGamete()
        ...
        if (signatureName.endsWith("ED25519")
                || signatureName.endsWith("EMPTY")
                || signatureName.endsWith("ED25519DET")) fileWithKeys = "gameteED25519.keys";
        else if (signatureName.endsWith("SHA256DSA")) fileWithKeys = "gameteSHA256DSA.keys";
        else if (signatureName.endsWith("QTESLA1") || signatureName.endsWith("QTESLA3"))
            fileWithKeys = "gameteQTesla.keys";
        else
            throw new NoSuchAlgorithmException(
                    "I have no keys for signing algorithm " + signatureName);
    ...
    private static void initializeNodeIfNeeded()
            throws TransactionRejectedException, TransactionException, CodeExecutionException,
        ...
        try {
            ...
            // if the original node has no manifest yet, it means that it is not initialized and we
            // initialize it
            ...
            Path takamakaCode =
                    Paths.get(
                            System.getProperty("user.home")
                                    + "/.m2/repository/io/hotmoka/io-takamaka-code/1.0.0/io-takamaka-code-"
                                    + takamakaVersion
                                    + ".jar");

            if (tendermintBlockchain != null)
                TendermintInitializedNode.of(
                        tendermintBlockchain, consensus, keysOfGamete, takamakaCode, aLot, aLot);
            else InitializedNode.of(node, consensus, keysOfGamete, takamakaCode, aLot, aLot);
        }
    ...
    private static Node mkMemoryBlockchain() throws NoSuchAlgorithmException {
        MemoryBlockchainConfig config =
                new MemoryBlockchainConfig.Builder()
                        .setMaxGasPerViewTransaction(_10_000_000)
                        .build();
    ...
    private static Node mkTakamakaBlockchainExecuteOneByOne() throws NoSuchAlgorithmException {
        TakamakaBlockchainConfig config =
                new TakamakaBlockchainConfig.Builder()
                        .setMaxGasPerViewTransaction(_10_000_000)
                        .build();
        nodeConfig = config;
        consensus =
        ...
        return takamakaBlockchain =
                TakamakaBlockchain.init(
                        config,
                        consensus,
                        TakamakaBlockchainOneByOne
                                ::postTransactionTakamakaBlockchainRequestsOneByOne);
    }

    // this code must stay in its own class, or otherwise the static initialization of TakamakaTest
    ...
    private static class TakamakaBlockchainOneByOne {
        ...
        /**
         * This simulates the implementation of postTransaction() in such a way to put each request
         * in a distinct delta group. By making this method synchronized, we avoid that two delta
         * groups get executed in parallel.
         *
         * @param request the request
         */
        private static synchronized void postTransactionTakamakaBlockchainRequestsOneByOne(
                TransactionRequest<?> request) {
            DeltaGroupExecutionResult result =
                    takamakaBlockchain.execute(
                            hash,
    ...
    private static class TakamakaBlockchainAtEachTimeslot {
        private static final List<TransactionRequest<?>> mempool = new ArrayList<>();

        /**
         * This simulates the implementation of postTransaction() in such a way to put each request
         * in a distinct delta group.
         *
         * @param request the request
         */
        private static void postTransactionTakamakaBlockchainRequestsOneByOne(
                TransactionRequest<?> request) {
    ...
    private static Node mkTakamakaBlockchainExecuteAtEachTimeslot()
        ...
        // we provide an implementation of postTransaction() that just adds the request in the
        // mempool
        takamakaBlockchain =
                TakamakaBlockchain.init(
                        config,
                        consensus,
                        TakamakaBlockchainAtEachTimeslot
                                ::postTransactionTakamakaBlockchainRequestsOneByOne);
        TakamakaBlockchain local = takamakaBlockchain;

        // we start a scheduler that checks the mempool every time-slot to see if there are requests
        // to execute
        Thread scheduler =
                new Thread(
                        () -> {
                            ...
                            while (true) {
                                ...
                                if (local.getCurrentExecutionId().isEmpty()) {
                                    ...
                                    synchronized (mempool) {
                                        ...
                                        if (mempoolSize == 0)
                                            // it is possible, but useless, to start an empty
                                            // execute()
                                            continue;

                                        // the clone of the mempool is needed or otherwise a
                                        // concurrent modification exception might occur later
                                    ...
                                    DeltaGroupExecutionResult result =
                                            local.execute(
                                                    ...
                                                    System.currentTimeMillis(),
                                                    requests,
                                                    Stream.generate(() -> BigInteger.ZERO)
                                                            .limit(size),
                                                    "id");
                                    hash = result.getHash();
                                    local.checkOut(hash);
    ...
    private static Node mkRemoteNode(Node exposed) {
        // we use port 8080, so that it does not interfere with the other service opened at port
        // 8081 by the network tests
        NodeServiceConfig serviceConfig =
                new NodeServiceConfig.Builder().setPort(8080).setSpringBannerModeOn(false).build();

        NodeService.of(serviceConfig, exposed);

        RemoteNodeConfig remoteNodeConfig =
                new RemoteNodeConfig.Builder()
    ...
    protected final void setAccounts(BigInteger... coins)
            throws InvalidKeyException, SignatureException, NoSuchAlgorithmException,
                    TransactionRejectedException, TransactionException, CodeExecutionException,
                    NoSuchElementException, ClassNotFoundException {
        nodeWithAccountsView =
                NodeWithAccounts.of(node, localGamete, privateKeyOfLocalGamete, coins);
    }

    protected final void setAccounts(
            String containerClassName, TransactionReference classpath, BigInteger... coins)
            throws InvalidKeyException, SignatureException, NoSuchAlgorithmException,
                    TransactionRejectedException, TransactionException, CodeExecutionException,
                    ClassNotFoundException {
        nodeWithAccountsView =
                NodeWithAccounts.of(
                        node,
                        localGamete,
                        privateKeyOfLocalGamete,
                        containerClassName,
                        classpath,
                        coins);
    }

    protected final void setAccounts(Stream<BigInteger> coins)
            throws InvalidKeyException, SignatureException, NoSuchAlgorithmException,
                    TransactionRejectedException, TransactionException, CodeExecutionException,
                    NoSuchElementException, ClassNotFoundException {
    ...
    protected static final NodeWithAccounts mkAccounts(Stream<BigInteger> coins)
            throws InvalidKeyException, SignatureException, NoSuchAlgorithmException,
                    TransactionRejectedException, TransactionException, CodeExecutionException,
                    NoSuchElementException, ClassNotFoundException {
        return NodeWithAccounts.of(
                node, localGamete, privateKeyOfLocalGamete, coins.toArray(BigInteger[]::new));
    }

    protected final void setAccounts(
            String containerClassName, TransactionReference classpath, Stream<BigInteger> coins)
            throws InvalidKeyException, SignatureException, NoSuchAlgorithmException,
                    TransactionRejectedException, TransactionException, CodeExecutionException,
                    ClassNotFoundException {
    ...
    protected final void setGreenRedAccounts(BigInteger... coins)
            throws TransactionRejectedException, TransactionException, CodeExecutionException,
                    InvalidKeyException, SignatureException, NoSuchAlgorithmException,
                    NoSuchElementException, ClassNotFoundException {
        nodeWithAccountsView =
                NodeWithAccounts.ofGreenRed(node, localGamete, privateKeyOfLocalGamete, coins);
    }

    protected static void setJar(String jar)
            throws InvalidKeyException, SignatureException, NoSuchAlgorithmException,
                    TransactionRejectedException, TransactionException, CodeExecutionException,
                    IOException, NoSuchElementException, ClassNotFoundException {
        TakamakaTest.jar =
                NodeWithJars.of(node, localGamete, privateKeyOfLocalGamete, pathOfExample(jar))
                        .jar(0);
    }
    ...
    protected final TransactionResponse getResponse(TransactionReference reference)
            throws NoSuchElementException, TransactionRejectedException {
        return node.getResponse(reference);
    }

    protected final TransactionReference addJarStoreInitialTransaction(
            byte[] jar, TransactionReference... dependencies) throws TransactionRejectedException {
        return node.addJarStoreInitialTransaction(
                new JarStoreInitialTransactionRequest(jar, dependencies));
    }

    /** Takes care of computing the next nonce. */
    protected final TransactionReference addJarStoreTransaction(
            PrivateKey key,
            ...
            throws TransactionException, TransactionRejectedException, InvalidKeyException,
                    SignatureException {
        return node.addJarStoreTransaction(
                new JarStoreTransactionRequest(
    ...
    /** Takes care of computing the next nonce. */
    protected final StorageReference addConstructorCallTransaction(
            PrivateKey key,
            StorageReference caller,
            BigInteger gasLimit,
            BigInteger gasPrice,
            ...
            StorageValue... actuals)
            throws TransactionException, CodeExecutionException, TransactionRejectedException,
    ...
    protected final StorageValue addInstanceMethodCallTransaction(
            ...
            throws TransactionException, CodeExecutionException, TransactionRejectedException,
                    InvalidKeyException, SignatureException {
        return node.addInstanceMethodCallTransaction(
                new InstanceMethodCallTransactionRequest(
                        Signer.with(signature, key),
                        caller,
                        getNonceOf(caller),
    ...
    /** Takes care of computing the next nonce. */
    protected final StorageValue addStaticMethodCallTransaction(
            PrivateKey key,
            StorageReference caller,
            BigInteger gasLimit,
            BigInteger gasPrice,
    ...
    /** Takes care of computing the next nonce. */
    protected final StorageValue runInstanceMethodCallTransaction(
            StorageReference caller,
            BigInteger gasLimit,
            TransactionReference classpath,
            MethodSignature method,
        ...
        return node.runInstanceMethodCallTransaction(
                new InstanceMethodCallTransactionRequest(
                        caller, gasLimit, classpath, method, receiver, actuals));
    }

    /** Takes care of computing the next nonce. */
    protected final StorageValue runStaticMethodCallTransaction(
            StorageReference caller,
            BigInteger gasLimit,
            TransactionReference classpath,
            MethodSignature method,
            StorageValue... actuals)
            throws TransactionException, CodeExecutionException, TransactionRejectedException {
        return node.runStaticMethodCallTransaction(
                new StaticMethodCallTransactionRequest(
                        caller, gasLimit, classpath, method, actuals));
    }

    /** Takes care of computing the next nonce. */
    protected final JarSupplier postJarStoreTransaction(
            PrivateKey key,
            StorageReference caller,
            BigInteger gasLimit,
            BigInteger gasPrice,
            ...
            byte[] jar,
            TransactionReference... dependencies)
            throws TransactionRejectedException, InvalidKeyException, SignatureException {
        return node.postJarStoreTransaction(
                new JarStoreTransactionRequest(
                        Signer.with(signature, key),
                        caller,
                        getNonceOf(caller),
    ...
    protected final CodeSupplier<StorageValue> postInstanceMethodCallTransaction(
            ...
            StorageValue... actuals)
            throws TransactionRejectedException, InvalidKeyException, SignatureException {
        return node.postInstanceMethodCallTransaction(
                new InstanceMethodCallTransactionRequest(
                        Signer.with(signature, key),
                        caller,
                        getNonceOf(caller),
    ...
    /** Takes care of computing the next nonce. */
    protected final CodeSupplier<StorageReference> postConstructorCallTransaction(
            PrivateKey key,
            StorageReference caller,
            BigInteger gasLimit,
            BigInteger gasPrice,
            ...
            StorageValue... actuals)
            throws TransactionRejectedException, InvalidKeyException, SignatureException {
        return node.postConstructorCallTransaction(
                new ConstructorCallTransactionRequest(
                        Signer.with(signature, key),
                        caller,
                        getNonceOf(caller),
    ...
    protected static void throwsTransactionExceptionWithCause(
            Class<? extends Throwable> expected, TestBody what) {
        try {
            what.run();
        } catch (TransactionException e) {
            if (e.getMessage().startsWith(expected.getName())) return;

            fail("wrong cause: expected " + expected.getName() + " but got " + e.getMessage());
        } catch (Exception e) {
            fail(
                    "wrong exception: expected "
    ...
    protected static void throwsTransactionExceptionWithCauseAndMessageContaining(
            Class<? extends Throwable> expected, String subMessage, TestBody what) {
        throwsTransactionExceptionWithCauseAndMessageContaining(
                expected.getName(), subMessage, what);
    }

    protected static void throwsTransactionExceptionWithCauseAndMessageContaining(
            String expected, String subMessage, TestBody what) {
        try {
            what.run();
        } catch (TransactionException e) {
            if (e.getMessage().startsWith(expected)) {
                if (e.getMessage().contains(subMessage)) return;
    ...
    protected static void throwsTransactionRejectedWithCause(
            Class<? extends Throwable> expected, TestBody what) {
        try {
            what.run();
            ...
            fail(
                    "wrong exception: expected "
                            + TransactionRejectedException.class.getName()
                            + " but got "
                            + e.getClass().getName());
        }
    ...
    protected static void throwsTransactionExceptionWithCause(String expected, TestBody what) {
        try {
            ...
            if (e.getMessage().startsWith(expected)) return;
    ...
    protected static void throwsVerificationExceptionWithMessageContaining(
            String subMessage, TestBody what) {
        throwsTransactionExceptionWithCauseAndMessageContaining(
                VerificationException.class, subMessage, what);
    }
     ...
     */
    protected final BigInteger getNonceOf(StorageReference account)
    ...
    /**
     * Gets the (green) balance of the given account. It calls the {@code
     * AccountWithAccessibleBalance.getBalance()} method.
     *
     * @param account the account
     * @return the balance
     * @throws TransactionRejectedException if the balance cannot be found
     */
    protected static BigInteger getBalanceOf(StorageReference account)
            throws TransactionRejectedException {
        try {
